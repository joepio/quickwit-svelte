...
import { serializeSortByField } from "./urlUtils";
...
export class Client {
  ...
  async search<ResultType>(
    request: SearchRequest
  ): Promise<SearchResponse<ResultType>> {
  ...
  async getAllSplits(indexId: string): Promise<Array<SplitMetadata>> {
    const splits: Array<SplitMetadata> = await this.fetch(
      `${this.apiRoot()}indexes/${indexId}/splits`,
      {}
    );
  ...
  buildSearchUrl(request: SearchRequest): URL {
    ...
    url.searchParams.append("max_hits", request.maxHits.toString() || "20");
    ...
    if (request.endTimestamp) {
      url.searchParams.append("end_timestamp", request.endTimestamp.toString());
    }
